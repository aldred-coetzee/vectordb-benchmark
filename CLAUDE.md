...
| 19 | QPS variance ±40% across efSearch sweeps | All DBs | No cache isolation: ef=128 cold, ef=256 warm from prior 10K queries. Also: ChromaDB `modify()` called per-query (10K extra round-trips). GIST 1K queries too few for stable QPS. | Pre-sweep cache warming (1K queries), scaled warmup, ChromaDB `_current_ef_search` cache, GIST padding to 5K, `--cold` flag |
