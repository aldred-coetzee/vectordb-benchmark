...
| 19 | QPS variance ±40% across efSearch sweeps | **Fixed** | Cache bias: ef=128 ran cold, ef=256 benefited from cached HNSW graph. Fix: pre-sweep cache warming (1K queries), scaled per-ef warmup, GIST query padding (1K→5K). Optional `--cold` flag for container restart between ef values. |
